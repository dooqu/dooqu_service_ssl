...
/*
template <class SOCK_TYPE>
bool ws_session<SOCK_TYPE>::confirm_ws_handshake_and_response(ws_request &req)
{
	bool upgrade_header = false, connect_header = false;
	const char *key = NULL, key1 = NULL, key2 = NULL;
...
template <class SOCK_TYPE>
void ws_session<SOCK_TYPE>::read_from_client()
{
	this->socket_.async_read_some(boost::asio::buffer(this->frame_data_.data, ws_framedata::BUFFER_SIZE),
								  std::bind(&ws_session<SOCK_TYPE>::on_data_received, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
}
...
template <class SOCK_TYPE>
void ws_session<SOCK_TYPE>::on_data_received(const boost::system::error_code &error, size_t bytes_received)
{
	if (!error)
	{
		do
...
template <class SOCK_TYPE>
bool ws_session<SOCK_TYPE>::alloc_available_buffer(buffer_stream **buffer_alloc)
{
	//如果当前的写入位置的指针指向存在一个可用的send_buffer，那么直接取这个集合；
	if (write_pos_ < this->send_buffer_sequence_.size())
	{
...
template <class SOCK_TYPE>
void ws_session<SOCK_TYPE>::write(char *data)
{
	this->write("%s", data);
}

template <class SOCK_TYPE>
void ws_session<SOCK_TYPE>::write(const char *format, ...)
{
	buffer_stream *curr_buffer = NULL;
	...
	if (read_pos_ == -1)
	{
		...
		++read_pos_;
		boost::asio::async_write(this->socket_, boost::asio::buffer(curr_buffer->read() + curr_buffer->pos_start, curr_buffer->size()),
								 std::bind(&ws_session<SOCK_TYPE>::send_handle, shared_from_this(), std::placeholders::_1));
	}
}

template <class SOCK_TYPE>
void ws_session<SOCK_TYPE>::write_frame(bool isFin, ws_framedata::opcode opcode, const char *format, ...)
{
	buffer_stream *curr_buffer = NULL;
...
template <class SOCK_TYPE>
void ws_session<SOCK_TYPE>::write_error(uint16_t error_code, char *error_reason)
{
	if (this->error_frame_sended_ == 0)
	{
		this->error_frame_sended_ = 1;
...
template <class SOCK_TYPE>
void ws_session<SOCK_TYPE>::send_handle(const boost::system::error_code &error)
{
	buffer_stream *curr_buffer = this->send_buffer_sequence_.at(this->read_pos_);
	if (curr_buffer->is_error_frame())
	{
...
/*
template <class SOCK_TYPE>
ws_session<SOCK_TYPE>::~ws_session()
{
	std::cout << "~ws_client" << std::endl;
	int size = this->send_buffer_sequence_.size();
	for (int i = 0; i < size; i++)
