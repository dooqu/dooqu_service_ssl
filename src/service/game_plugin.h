...
public:
    ...
    template<class T> friend class game_service;
    friend class game_zone;

private:
    void load();
    void unload();
    int auth_client(ws_client* client, const std::string&);
    int join_client(ws_client* client);
    void remove_client(ws_client* client);
    void dispatch_bye(ws_client* client);
    void begin_update_client(ws_client* client, string& server_url, string& request_path);
    void end_update_client(const boost::system::error_code& err, const int status_code, const string& result, ws_client* client);
    void remove_client_from_plugin(ws_client* client);
...
protected:
    ...
    virtual int on_auth_client(ws_client* client, const std::string&);
    ...
    virtual void on_client_command(ws_client* client, command* command);
    ...
    virtual bool need_update_offline_client(ws_client* client, string& server_url, string& request_path);
    //如果当game_client离开game_plugin需要调用http协议的外部地址来更新更新game_client的状态；
    //那么请在此函数返回true，并且正确的赋值server_url和request_path的值；
    virtual void on_update_client(ws_client* client, const string& response_string);

	void queue_task(std::function<void(void)> callback_handle, int delay_duration, bool cancel_enabled = false);

public:

    game_plugin(ws_service* game_service, char* gameid, char* title, int capacity);
